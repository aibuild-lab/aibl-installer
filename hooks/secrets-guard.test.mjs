...
const CASES = [
  ...
  // Help-only discovery must work without opening a value-bearing command path.
  ['Bash', 'infisical secrets --help', 'allow'],
  ['Bash', '/opt/homebrew/bin/infisical secrets -h', 'allow'],
  ['Bash', 'infisical secrets get --help', 'allow'],
  ['Bash', 'infisical secrets folders get --help', 'allow'],
  ['Bash', 'infisical secrets set --help', 'allow'],
  ['Bash', 'infisical secrets delete --help', 'allow'],
  ['Bash', 'infisical secrets agent-proxy --help', 'allow'],
  ['Bash', 'git status && infisical secrets --help', 'allow'],
  ['Bash', "bash -c 'infisical secrets --help'", 'allow'],
  ['PowerShell', 'infisical secrets --help', 'allow'],
  ['PowerShell', '& "C:\\Program Files\\Infisical\\infisical" secrets get --help', 'allow'],
  ['Bash', 'infisical secrets --help=false', 'deny'],
  ['Bash', 'infisical secrets --help true', 'deny'],
  ['Bash', 'infisical secrets --help get API_KEY', 'deny'],
  ['Bash', 'infisical secrets get API_KEY --help', 'deny'],
  ['Bash', 'infisical secrets -- --help', 'deny'],
  ['Bash', 'infisical secrets --env=prod --help', 'deny'],
  ['Bash', 'infisical secrets --token=PASTE_YOUR_TOKEN_HERE --help', 'deny'],
  ['Bash', 'infisical secrets agent-proxy run --no-sandbox --help', 'deny'],
  ['Bash', 'infisical secrets --help; infisical secrets get API_KEY', 'deny'],
  ['Bash', 'infisical secrets --help && cat .env', 'deny'],
  ['Bash', 'infisical secrets --help | infisical secrets', 'deny'],
  ['Bash', "bash -c 'infisical secrets --help; printenv'", 'deny'],
  ['Bash', 'infisical secrets --help $(cat .env)', 'deny'],
  ['Bash', 'infisical dynamic-secrets --help', 'deny'],
  ['Bash', 'infisical pam --help', 'deny'],
  ...
  ['Bash', 'infisical secrets agent-proxy run --sandbox=false -- codex', 'deny'],
  ['Bash', 'infisical secrets agent-proxy run --sandbox false -- codex', 'deny'],
  ['Bash', 'infisical secrets agent-proxy run --sandbox=0 -- codex', 'deny'],
  ['Bash', 'infisical secrets agent-proxy run --sandbox=true -- codex', 'allow'],
  ...
  ['Bash', 'infisical dynamic-secrets', 'deny'],
  ['Bash', 'infisical dynamic-secrets lease create aws-daily --output json', 'deny'],
  ['Bash', '/opt/homebrew/bin/infisical dynamic-secrets lease revoke lease-id', 'deny'],
  ['Bash', 'infisical pam access prod/orders-db', 'deny'],
  ['Bash', 'infisical pam agentic access -- codex', 'deny'],
  ['Bash', 'infisical pam agentic access --no-sandbox -- codex', 'deny'],
  ['Bash', 'infisical user --token=PASTE_YOUR_TOKEN_HERE', 'deny'],
  ['Bash', 'infisical login --token PASTE_YOUR_TOKEN_HERE', 'deny'],
  ...
  ['Bash', "docker image inspect --format '{{json .Id}}' api", 'allow'],
  ['Bash', "docker image inspect -f '{{json .RepoDigests}}' api", 'allow'],
  ['Bash', "docker image inspect --format='{{json .Architecture}}' api", 'allow'],
  ['Bash', "docker image inspect --format '{{json .Config.User}}' api", 'allow'],
  ['Bash', "docker image inspect --format '{{json (index .Config.Labels \"org.opencontainers.image.revision\")}}' api", 'allow'],
  ['Bash', "docker image inspect --format '{{json (index .Config.Labels \"io.ragnos.knowledge.product-revision\")}}' api", 'allow'],
  ['Bash', "docker image inspect --format '{\"Id\":{{json .Id}},\"RepoDigests\":{{json .RepoDigests}},\"Architecture\":{{json .Architecture}},\"RuntimeUser\":{{json .Config.User}},\"DistributionSource\":{{json (index .Config.Labels \"org.opencontainers.image.revision\")}},\"ProductSource\":{{json (index .Config.Labels \"io.ragnos.knowledge.product-revision\")}}}' api", 'allow'],
  ['Bash', "docker inspect --format '{{.Config.Env}}' api", 'deny'],
  ['Bash', "docker inspect --format '{{json .Config.Env}}' api", 'deny'],
  ['Bash', "docker inspect --format '{{json .Config}}' api", 'deny'],
  ['Bash', "docker inspect --format '{{json .Config.Labels}}' api", 'deny'],
  ['Bash', "docker inspect --format '{{json (index .Config \"Env\")}}' api", 'deny'],
  ['Bash', "docker inspect --format '{{json (index .Config.Labels \"example.private\")}}' api", 'deny'],
  ['Bash', "docker inspect --format '{{json .Id' api", 'deny'],
  ['Bash', "docker inspect --format '{{json .Id}}' --format '{{json .Architecture}}' api", 'deny'],
  ['Bash', 'docker image inspect --format "{{json .Id}}$(docker inspect secret)" api', 'deny'],
  ['Bash', 'docker image inspect --format "{{json .Id}}$(cat .env)" api', 'deny'],
  ['Bash', 'docker image inspect --format "{{json .Id}}$(printenv)" api', 'deny'],
  ['Bash', 'docker image inspect --format "{{json .Id}}$(op read op://Vault/Item/credential)" api', 'deny'],
  ['Bash', 'docker image inspect --format "$(docker inspect secret){{json .Id}}" api', 'deny'],
  ['Bash', 'docker image inspect --format "{{json .Id}}`docker inspect secret`" api', 'deny'],
  ['Bash', "docker image inspect --format '{{json .Id}}' api $(docker inspect secret)", 'deny'],
  ['Bash', "docker image inspect --format '{{json .Id}}' api <(docker inspect secret)", 'deny'],
  ['Bash', "docker image inspect --format '{{json .Id}}' api\ndocker inspect secret", 'deny'],
  ['Bash', "docker image inspect --format '{{json .Id}}' api > inspect.json", 'deny'],
  ['Bash', "bash -c 'docker image inspect --format \"{{json .Id}}\" api'", 'deny'],
  ['Bash', "sudo docker image inspect --format '{{json .Id}}' api", 'deny'],
  ['Bash', "docker exec helper docker image inspect --format '{{json .Id}}' api", 'deny'],
  ['Bash', "ssh host docker image inspect --format '{{json .Id}}' api", 'deny'],
  ['Bash', "podman inspect --format '{{json .Id}}' api", 'deny'],
  ...
  // systemd uses --plain only to suppress tree glyphs. The --plain deny now keys on the binary:
  // it fires only for secret-store CLIs, so any systemctl selector is allowed.
  ...
  ['Bash', 'systemctl list-units --type=service --state=failed --no-legend --no-pager --plain', 'allow'],
  ['Bash', 'systemctl --plain list-units', 'allow'],
  ['Bash', 'systemctl list-units --type=service --plain --no-legend', 'allow'],
  ['Bash', 'vault kv get --plain secret/example', 'deny'],
  ['Bash', 'infisical export --plain', 'deny'],
  ['Bash', 'doppler secrets get EXAMPLE --plain', 'deny'],
  ['Bash', 'op item get example --plain', 'deny'],
  ['Bash', 'chamber read svc key --plain=true', 'deny'],
  ['Bash', 'bash -c "doppler secrets get EXAMPLE --plain"', 'deny'],
  ['Bash', 'ssh box "vault kv get --plain secret/example"', 'deny'],
  ['Bash', 'sudo vault kv get --plain secret/example', 'deny'],
  ['Bash', 'op run -- doppler secrets get EXAMPLE --plain', 'deny'],
  ['Bash', `python3 -c "import os; os.system('doppler secrets get EXAMPLE --plain')"`, 'deny'],
  ['Bash', `node -e "require('child_process').execSync('vault kv get --plain secret/example')"`, 'deny'],
  ['Bash', 'xargs -I{} doppler secrets get {} --plain < names.txt', 'deny'],
  // Names-only env listing: only the NUL-record form, `env -0 | cut -z -d= -f1`. With newline
  // records a multi-line value (a PEM key) spills lines that `cut` prints in full, so the
  // newline form stays denied, with or without `-s`.
  ['Bash', 'env -0 | cut -z -d= -f1', 'allow'],
  ['Bash', 'env --null | cut --zero-terminated -d = -f 1', 'allow'],
  ['Bash', "env -0 | cut -z -d '=' -f1 | tr '\\0' '\\n' | sort", 'allow'],
  ['Bash', 'env -0 | cut -z --delimiter== --fields=1', 'allow'],
  ['Bash', 'env | cut -d= -f1', 'deny'],
  ['Bash', 'env | cut -d = -f 1 | sort', 'deny'],
  ['Bash', 'env | cut -s -d= -f1', 'deny'],
  ['Bash', 'printenv | cut -f1 -d=', 'deny'],
  ['Bash', 'env | cut -z -d= -f1', 'deny'],
  ['Bash', 'env -0 | cut -d= -f1', 'deny'],
  ['Bash', 'env -0 > env.txt | cut -z -d= -f1', 'deny'],
  ['Bash', 'env -0 1>env.txt | cut -z -d= -f1', 'deny'],
  ['Bash', 'env -0 1>&2 | cut -z -d= -f1', 'deny'],
  ['Bash', 'env > env.txt | cut -d= -f1', 'deny'],
  ['Bash', 'env -0', 'deny'],
  ['Bash', 'printenv -0', 'deny'],
  ['Bash', 'printenv --null', 'deny'],
  ['Bash', 'printenv --', 'deny'],
  ['Bash', 'printenv -0 --', 'deny'],
  ['Bash', 'printenv -0>env.txt', 'deny'],
  ['Bash', 'env -0 | sort -z', 'deny'],
  ['Bash', 'env -0 -i | cut -z -d= -f1', 'deny'],
  ['Bash', 'env | sort', 'deny'],
  ['Bash', 'env -0 | cut -z -d= -f2', 'deny'],
  ['Bash', 'env -0 | cut -z -d= -f1-', 'deny'],
  ['Bash', 'env -0 | cut -z -d= -f1,2', 'deny'],
  ['Bash', 'env -0 | cut -z -d= -f1 --complement', 'deny'],
  ['Bash', 'env -0 | cut -z -d: -f1', 'deny'],
  ['Bash', 'env -0 | cut -z -c1-200', 'deny'],
  ['Bash', 'env -0 | grep -z FOO | cut -z -d= -f1', 'deny'],
  ['Bash', 'env -0 | tee env.txt | cut -z -d= -f1', 'deny'],
  ['Bash', 'cut -z -d= -f1 names.txt; env -0', 'deny'],
  ['Bash', 'env -i', 'deny'],
  ['Bash', 'env -i HOME="$HOME" PATH="$PATH" make build', 'allow'],
  // `docker buildx inspect` describes a builder instance, not a container or image.
  ['Bash', 'docker buildx inspect mybuilder', 'allow'],
  ['Bash', 'docker buildx inspect --bootstrap mybuilder', 'allow'],
  ['Bash', 'docker buildx ls', 'allow'],
  ['Bash', 'docker container inspect api', 'deny'],
  ['Bash', 'docker image inspect api', 'deny'],
  ['Bash', 'docker buildx imagetools inspect ghcr.io/example/app:tag', 'deny'],
  // The names-only and builder allowances leave the other environment reads blocked.
  ['Bash', 'cat /proc/1/environ', 'deny'],
  ['Bash', 'ps eww', 'deny'],
