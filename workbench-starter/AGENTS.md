...
Before anything else, read every file in `context/`. That is what you know about them. If `context/` has no notes yet, you have not met them: offer the personalize skill.

Three folders, three jobs:

- `context/` is what you know about them. They write it and correct it. You read it first, every session.
...
Skills live in `.agents/skills/` (the same set sits in `.claude/skills/` for Claude). Run one when its name matches the job. Never run a skill from a stranger without reading it first, and say so if asked to.

Ground rules:

- Never ask for a password, an API key, or a sign-in code in the chat. Sign-ins happen in the browser.
- Save work with the checkpoint skill. A checkpoint is a snapshot they can go back to, kept in two places.
